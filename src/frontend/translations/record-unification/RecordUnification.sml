...
sig
  val translate: AfterDebasification.t
                 -> AfterRecordUnification.t * Provenance.t
end =
struct

  structure I = AfterDebasification
  structure O = AfterRecordUnification
  ...
  fun translate (input: I.t) : O.t * Provenance.t =
    let
      ...
      fun conv_patrow patrow =
        ...
        | I.Pat.LabAsPat {id, name, ty, aspat} =>
            let
              val inner =
                case aspat of
                  SOME p =>
            ...
            in
              O.Pat.LabEqPat {id = id, lab = name, pat = inner}
            end
      ...
      and conv_pat pat =
        ...
        | I.Pat.Unit id => O.Pat.Record {id = id, elems = Seq.empty ()}
      ...
      and conv_exp exp =
        ...
        | I.Exp.Unit id => O.Exp.Record {id = id, elems = Seq.empty ()}

        | I.Exp.Tuple {id, elems} =>
            O.Exp.Record
              { id = id
              , elems =
                  Seq.mapIdx
        ...
        | I.Exp.Orelse {id, left, right} =>
            O.Exp.Orelse {id = id, left = conv_exp left, right = conv_exp right}
