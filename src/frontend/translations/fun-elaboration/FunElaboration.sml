structure FunElaboration:
sig
  val translate: AfterInfixElaboration.t -> AfterFunElaboration.t * Provenance.t
end =
struct

  structure I = AfterInfixElaboration
  structure O = AfterFunElaboration

  fun translate (input: I.t) : O.t * Provenance.t =
    let
      val pass = "FunElaboration"
      ...
      and conv_dec (dec: I.Exp.dec) : O.Exp.dec =
        ...
        | I.Exp.DecFun {id, tyvars, fvalbind} =>
            O.Exp.DecVal
              {id = id, tyvars = tyvars, elems = conv_fvalbind fvalbind}
      ...
      (* Build fn p1 => fn p2 => ... => fn pn => body *)
      and wrap_lambdas (origin: NodeID.t) (pats: O.Pat.t list)
        (body: O.Exp.exp) : O.Exp.exp =
        List.foldr
          (fn (p, acc) =>
             O.Exp.Fn
               {id = synth origin "fun arg lambda", elems = Seq.% [{pat = p, exp = acc}]})
          body pats

      (* Translate one fvalbind group (one fun-binding) to a val rec element *)
      and conv_fun_group
        (clauses: {fname_args: I.Exp.fname_args, ty: I.Ty.t option, exp: I.Exp.exp} Seq.t) :
        {is_rec: bool, pat: O.Pat.t, exp: O.Exp.exp} =
        let
          val first = Seq.nth clauses 0
          val {id = fa_id, name, args = first_args} = #fname_args first
          val arity = Seq.length first_args
          val num_clauses = Seq.length clauses

          val name_pat =
            O.Pat.Ident {id = synth fa_id "fun name pattern", name = Seq.% [name]}

          fun make_body
            ({fname_args = fa, ty, exp}: { fname_args: I.Exp.fname_args
                                         , ty: I.Ty.t option
                                         , exp: I.Exp.exp
                                         }) =
            let
            ...
            in
              ...
              | SOME t =>
                  O.Exp.Typed
                    {id = synth (#id fa) "fun return type", exp = e, ty = conv_ty t}
            end

          val fn_exp =
            if num_clauses = 1
            then
              (* Single clause: fn p1 => fn p2 => ... => fn pn => body *)
              wrap_lambdas fa_id
                (List.map conv_pat (Seq.toList first_args))
                (make_body first)
            else
              let
                val fresh_names =
                  List.tabulate (arity, fn i => "_x" ^ Int.toString (i + 1))

                fun var_ident nm =
                  O.Exp.Ident {id = synth fa_id "fun case var", name = Seq.% [nm]}

                (* Scrutinee: _x1 for arity 1; {1=_x1,...} for arity > 1 *)
                val scrutinee =
                  if arity = 1
                  then var_ident (List.nth (fresh_names, 0))
                  else
                    let
                      fun make_rows [] _ = []
                        | make_rows (nm :: rest) i =
                            O.Exp.RecordRow
                              { id = synth fa_id
                                  ("fun scrutinee field " ^ Int.toString i)
                              , lab = Int.toString i
                              , exp = var_ident nm
                              }
                            :: make_rows rest (i + 1)
                    in
                      O.Exp.Record
                        { id = synth fa_id "fun multi-arg scrutinee"
                        , elems = Seq.% (make_rows fresh_names 1)
                        }
                    end

                (* Case pattern for one clause: p for arity 1; {1=p,...} for arity > 1 *)
                fun make_case_pat (clause_fa: I.Exp.fname_args) : O.Pat.t =
                  let
                    val args = Seq.toList (#args clause_fa)
                    val clause_id = #id clause_fa
                  in
                    if arity = 1
                    then conv_pat (List.nth (args, 0))
                    else
                      let
                        fun make_rows [] _ = []
                          | make_rows (p :: rest) i =
                      ...
                      in
                        O.Pat.Record
                          { id = synth clause_id "fun multi-arg case pattern"
                          , elems = Seq.% (make_rows args 1)
                          }
                      end
                  end
                ...
                val case_exp =
                  O.Exp.Case
                    { id = synth fa_id "fun multi-clause case"
                    , exp = scrutinee
                    , elems =
                        Seq.map
                          (fn clause =>
                             { pat = make_case_pat (#fname_args clause)
                             , exp = make_body clause
                             })
                          clauses
                    }

                fun make_arg_pats [] = []
                  | make_arg_pats (nm :: rest) =
                      O.Pat.Ident
                        { id = synth fa_id "fun lambda arg pat"
                        , name = Seq.% [nm]
                        }
                      :: make_arg_pats rest
              in
                wrap_lambdas fa_id (make_arg_pats fresh_names) case_exp
              end
        in
          {is_rec = true, pat = name_pat, exp = fn_exp}
        end

      and conv_fvalbind (fvb: I.Exp.exp I.Exp.fvalbind) :
        {is_rec: bool, pat: O.Pat.t, exp: O.Exp.exp} Seq.t =
        Seq.map
          (fn {elems = clauses} => conv_fun_group clauses)
          (#elems fvb)
