  ...
  fun to_source_ast_mlb (top_src: Source.t) (top_ast: MLBAst.t)
    ...
    let
      ...
      fun convert_basexp (mlb_src: Source.t) (infdict: InfixDict.t)
        ...
        case bexp of
          MLBAst.Ident tok =>
            (infdict, SourceAst.Mlb.Ident
              { id = fresh_node (MLBToken.getSource tok)
              , name = tok_str tok
              })
        | MLBAst.LetInEnd {lett, basdec, inn = _, basexp, endd = _} =>
            let
              val (infdict1, basdec') = convert_basdec mlb_src infdict basdec
              val (infdict2, basexp') = convert_basexp mlb_src infdict1 basexp
            in
              (infdict2, SourceAst.Mlb.LetInEnd
      ...
      and convert_basdec (mlb_src: Source.t) (infdict: InfixDict.t)
        ...
        | MLBAst.DecMultiple {elems, delims = _} =>
            let
              val (infdict', rev_elems) = Seq.iterate
                (fn ((d, acc), elem) =>
                ...
                elems
            in
              (infdict', SourceAst.Mlb.DecMultiple
        ...
        | MLBAst.DecPathSML {path, token = _} =>
            let
              ...
              val (infdict', ast) = Parser.parse_sml_with_infdict infdict sml_src
              val sml = ToSourceAstSML.convert fresh_node sml_src ast
            in
              (infdict', SourceAst.Mlb.DecSml {id = fresh_node sml_src, sml = sml})
            end
        ...
        | MLBAst.DecPathMLB {path, token = _} =>
            ...
            in
              case find_visited abs_path of
                SOME name =>
                  (infdict, SourceAst.Mlb.DecRef {id = fresh_node mlb_src, name = name})
              | NONE =>
                  let
                    ...
                    val (infdict', nested_basdec) =
                      convert_basdec nested_src infdict nested_dec
        ...
        | MLBAst.DecBasis {basis, elems, delims = _} =>
            let
              val (infdict', rev_elems) = Seq.iterate
                ...
                elems
            in
              (infdict', SourceAst.Mlb.DecBasis
        ...
        | MLBAst.DecLocalInEnd {locall, basdec1, inn = _, basdec2, endd = _} =>
            let
              val (infdict1, basdec1') = convert_basdec mlb_src infdict basdec1
              val (infdict2, basdec2') = convert_basdec mlb_src infdict1 basdec2
            in
              (infdict2, SourceAst.Mlb.DecLocalInEnd
        ...
        | MLBAst.DecSignature {signaturee, elems, delims = _} =>
            (infdict, SourceAst.Mlb.DecSignature
        ...
        | MLBAst.DecFunctor {functorr, elems, delims = _} =>
            (infdict, SourceAst.Mlb.DecFunctor
        ...
        | MLBAst.DecUnderscorePrim tok =>
            (infdict, SourceAst.Mlb.DecUnderscorePrim
              (fresh_node (MLBToken.getSource tok)))

      val MLBAst.Ast top_basdec = top_ast
      val (_, main) = convert_basdec top_src InfixDict.initialTopLevel top_basdec
