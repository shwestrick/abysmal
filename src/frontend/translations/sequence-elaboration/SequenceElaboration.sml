structure SequenceElaboration:
sig
  val translate: AfterWhileElaboration.t -> AfterSequenceElaboration.t * Provenance.t
end =
struct

  structure I = AfterWhileElaboration
  structure O = AfterSequenceElaboration

  fun translate (input: I.t) : O.t * Provenance.t =
    let
      val pass = "SequenceElaboration"
      ...
      (* Build val _ = e, used when discarding a sequence element *)
      fun make_ignore_dec (origin: NodeID.t) (e: O.Exp.exp) : O.Exp.dec =
        O.Exp.DecVal
          { id = synth origin "seq ignore dec"
          , tyvars = Seq.empty ()
          , elems = Seq.%
              [{is_rec = false, pat = O.Pat.Wild (synth origin "seq wild"), exp = e}]
          }

      (* Combine a list of decs into one, avoiding unnecessary DecMultiple *)
      fun combine_decs (origin: NodeID.t) (decs: O.Exp.dec list) : O.Exp.dec =
        case decs of
          [] => O.Exp.DecEmpty
        | [d] => d
        | _ => O.Exp.DecMultiple {id = synth origin "seq decs", elems = Seq.% decs}
      ...
      fun conv_exp (exp: I.Exp.exp) : O.Exp.exp =
        ...
        | I.Exp.Sequence {id, elems} =>
            (case Seq.toList elems of
               [] =>
                 O.Exp.Record {id = synth id "empty seq unit", elems = Seq.empty ()}
             | [e] => conv_exp e
             | es =>
                 let
                   val n = List.length es
                   val prefix = List.take (es, n - 1)
                   val last = List.nth (es, n - 1)
                   val dec =
                     combine_decs id (List.map (make_ignore_dec id o conv_exp) prefix)
                 in
                   O.Exp.LetInEnd
                     {id = synth id "seq let", dec = dec, exp = conv_exp last}
                 end)
        | I.Exp.LetInEnd {id, dec, exps} =>
            (case Seq.toList exps of
               [] =>
                 O.Exp.LetInEnd
                   { id = id
                   , dec = conv_dec dec
                   , exp = O.Exp.Record
                       {id = synth id "empty let exps unit", elems = Seq.empty ()}
                   }
             | [e] =>
                 O.Exp.LetInEnd {id = id, dec = conv_dec dec, exp = conv_exp e}
             | es =>
                 let
                   val n = List.length es
                   val prefix = List.take (es, n - 1)
                   val last = List.nth (es, n - 1)
                   val extra_decs =
                     List.map (make_ignore_dec id o conv_exp) prefix
                   val combined_dec =
                     O.Exp.DecMultiple
                       { id = synth id "let seq decs"
                       , elems = Seq.% (conv_dec dec :: extra_decs)
                       }
                 in
                   O.Exp.LetInEnd {id = id, dec = combined_dec, exp = conv_exp last}
                 end)
