structure AfterRecordUnification =
struct
  ...
  (** =========================================================================
    ...
    *   - Tuple removed (elaborated to Record with numeric labels "1", "2", ...)
    *)
  structure Ty =
  struct
    datatype ty =
  ...
  (** =========================================================================
    ...
    *
    * Changes from SourceAst:
    *   - patrow.LabAsPat removed (elaborated to LabEqPat)
    *   - Unit removed (elaborated to Record with empty elems)
    *   - Tuple removed (elaborated to Record with numeric labels "1", "2", ...)
    *)
  structure Pat =
  struct
    ...
    (** [op] longvid *)
    | Ident of {id: node_id, has_op: bool, name: longid}
    ...
    (** [op] longvid atpat *)
    | Con of {id: node_id, has_op: bool, name: longid, atpat: pat}

    (** pat vid pat *)
    | Infix of {id: node_id, left: pat, opr: string, right: pat}
  ...
  (** =========================================================================
    ...
    *
    * Changes from SourceAst:
    *   - row_exp.RecordPun removed (elaborated to RecordRow)
    *   - Unit removed (elaborated to Record with empty elems)
    *   - Tuple removed (elaborated to Record with numeric labels "1", "2", ...)
    *)
  structure Exp =
  struct
    ...
    datatype 'exp row_exp =
      RecordRow of {id: node_id, lab: string, exp: 'exp}
    ...
    (** ( exp; ...; exp ) *)
    | Sequence of {id: node_id, elems: exp Seq.t}

    (** let dec in exp [; exp ...] end *)
    | LetInEnd of {id: node_id, dec: dec, exps: exp Seq.t}

    (** exp exp *)
    | App of {id: node_id, left: exp, right: exp}

    (** exp vid exp *)
    | Infix of {id: node_id, left: exp, opr: string, right: exp}

    (** exp : ty *)
    | Typed of {id: node_id, exp: exp, ty: Ty.t}

    (** exp andalso exp *)
    | Andalso of {id: node_id, left: exp, right: exp}
    ...
    | IfThenElse of {id: node_id, exp1: exp, exp2: exp, exp3: exp}
    ...
    (** infix [d] vid [vid ...] *)
    | DecInfix of {id: node_id, precedence: int option, elems: string Seq.t}

    (** infixr [d] vid [vid ...] *)
    | DecInfixr of {id: node_id, precedence: int option, elems: string Seq.t}

    (** nonfix vid [vid ...] *)
    | DecNonfix of {id: node_id, elems: string Seq.t}
  ...
  datatype sml_ast = SmlAst of {id: node_id, topdecs: topdec Seq.t}
  ...
  struct
    ...
    | LetInEnd of {id: node_id, basdec: basdec, basexp: basexp}

    (** bas basdec end *)
    | BasEnd of {id: node_id, basdec: basdec}
    ...
    (** open basid ... basid *)
    | DecOpen of {id: node_id, elems: string Seq.t}
