structure WhileElaboration:
sig
  val translate: AfterFnElaboration.t -> AfterWhileElaboration.t * Provenance.t
end =
struct

  structure I = AfterFnElaboration
  structure O = AfterWhileElaboration

  fun translate (input: I.t) : O.t * Provenance.t =
    let
      val pass = "WhileElaboration"
      ...
      fun conv_exp (exp: I.Exp.exp) : O.Exp.exp =
        ...
        | I.Exp.While {id, exp1, exp2} =>
             ...
             *   while e1 do e2
             * →
             *   let val rec _loop = fn () => case e1 of true => (e2; _loop ()) | _ => ()
             *   in _loop () end
             *)
            let
              val unit_exp = O.Exp.Record {id = synth id "while unit", elems = Seq.empty ()}
              val unit_pat = O.Pat.Record {id = synth id "while unit pat", elems = Seq.empty ()}
              val loop_name = "_loop"
              val loop_ref_id = synth id "while loop ref"
              fun loop_call () =
                O.Exp.App
                  { id = synth id "while loop call"
              ...
              val case_exp =
                O.Exp.Case
                  { id = synth id "while case"
                  , exp = conv_exp exp1
                  , elems = Seq.%
                      [ { pat = O.Pat.Ident {id = synth id "while true pat", name = Seq.% ["true"]}
                        , exp = body_exp
                        }
                      , { pat = O.Pat.Wild (synth id "while default pat")
                        , exp = unit_exp
                        }
                      ]
                  }
              val loop_fn =
                O.Exp.Fn
                  { id = synth id "while loop fn"
                  , pat = unit_pat
                  , exp = case_exp
                  }
              val loop_dec =
                O.Exp.DecVal
                  { id = synth id "while loop dec"
                  , tyvars = Seq.empty ()
                  , elems = Seq.%
                      [ { is_rec = true
                        , pat = O.Pat.Ident
                            { id = synth id "while loop pat"
                            , name = Seq.% [loop_name]
                            }
                        , exp = loop_fn
                        }
